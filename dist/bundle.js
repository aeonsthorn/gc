...
function create(name, options) {
  return __async(this, null, function* () {
    ...
    try {
      ...
      console.error(import_chalk.default.red(e));
      return;
    }
    if (!options.quiet) {
      logOutput(dirName, componentName);
...
var okLog = (...args) => console.log(import_chalk.default.green(...args));
...
function createFileFromTemplate(dirName, componentName, fileSuffix) {
  return __async(this, null, function* () {
    const templateFileContent = yield import_fs.promises.readFile(path.join(__dirname, "..", `template/typescript/__Component__${fileSuffix}`), { encoding: "utf-8" });
...
function createIndexFile(dirName, componentName) {
  return __async(this, null, function* () {
    const indexFileContent = yield import_fs.promises.readFile(path.join(__dirname, "..", "template/typescript/index.ts"), { encoding: "utf-8" });
...
program2.command("create").description("Create a new component").argument("<name>", "Name for the component").option("--dry-run", "Dry run").option("--js", "Javascript version").option("--quiet", "Quiet mode").action(create_default);
program2.command("rename").description("Rename a component").argument("<oldName>", "Name for the component").argument("<newName>", "New name for the component").action((oldName, newName) => {
  console.log({ oldName });
  console.log({ newName });
});
program2.command("delete").description("Delete a component").argument("<name>", "Name for the component").action((name) => {
  console.log({ name });
});
